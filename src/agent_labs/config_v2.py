...
class AgentConfig(BaseModel):
    ...
    def _load_from_env() -> Dict[str, Any]:
        ...
        if os.getenv("MEMORY_SHORT_TERM_SIZE"):
            config.setdefault("memory", {})["short_term_size"] = int(os.getenv("MEMORY_SHORT_TERM_SIZE"))
        if os.getenv("MEMORY_LONG_TERM_ENABLED"):
            config.setdefault("memory", {})["long_term_enabled"] = os.getenv("MEMORY_LONG_TERM_ENABLED").lower() == "true"
        if os.getenv("MEMORY_CONTEXT_WINDOW"):
            config.setdefault("memory", {})["context_window"] = int(os.getenv("MEMORY_CONTEXT_WINDOW"))
        ...
        if os.getenv("AGENT_ENABLE_REFLECTION"):
            config.setdefault("engine", {})["enable_reflection"] = os.getenv("AGENT_ENABLE_REFLECTION").lower() == "true"
        
        # Observability section
        if os.getenv("OBSERVABILITY_ENABLE_TRACING"):
            config.setdefault("observability", {})["enable_tracing"] = os.getenv("OBSERVABILITY_ENABLE_TRACING").lower() == "true"
        if os.getenv("OBSERVABILITY_ENABLE_METRICS"):
            config.setdefault("observability", {})["enable_metrics"] = os.getenv("OBSERVABILITY_ENABLE_METRICS").lower() == "true"
        if os.getenv("OBSERVABILITY_LOG_PROMPTS"):
            config.setdefault("observability", {})["log_prompts"] = os.getenv("OBSERVABILITY_LOG_PROMPTS").lower() == "true"
        if os.getenv("OBSERVABILITY_LOG_RESPONSES"):
            config.setdefault("observability", {})["log_responses"] = os.getenv("OBSERVABILITY_LOG_RESPONSES").lower() == "true"
